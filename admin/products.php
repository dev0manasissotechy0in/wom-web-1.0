...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $slug = generateSlug($_POST['slug'] ?: $product_name);
...
$products = $db->query("SELECT * FROM products ORDER BY display_order")->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
                ...
                <form method="POST" class="form" style="padding: 20px;">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="product_name" class="form-control" value="<?php echo $edit_product ? ($edit_product['product_name']) : ''; ?>" required>
                        </div>
                        
                        <div class="form-group">
                            <label>Price (₹) <span class="required">*</span></label>
                            <input type="number" name="price" class="form-control" step="0.01" value="<?php echo $edit_product ? $edit_product['price'] : ''; ?>" required>
                        </div>
                    </div>
                    
                    <div class="form-group">
                        <label>Slug</label>
                        <input type="text" name="slug" class="form-control" value="<?php echo $edit_product ? ($edit_product['slug']) : ''; ?>">
                    </div>
                    
                                                             <div class="form-group">
                        <label>Short Description <span class="required">*</span></label>
                        <textarea name="short_description" class="form-control" rows="3" required><?php echo $edit_product ? ($edit_product['short_description']) : ''; ?></textarea>
                    </div>
                    ...
                    <div class="form-group">
                        <label>BUTTON URL<span class="required">*</span></label>
                        <textarea name="url" class="form-control" rows="3" required><?php echo $edit_product ? ($edit_product['url']) : ''; ?></textarea>
                    </div>
                    
                    <div class="form-group">
                        <label>Features (comma separated) <span class="required">*</span></label>
                        <textarea name="features" class="form-control" rows="3" required><?php echo $edit_product ? implode(', ', json_decode($edit_product['features'], true)) : ''; ?></textarea>
                        <small>e.g., Feature 1, Feature 2, Feature 3</small>
                    </div>
                    
                    <div class="form-group">
                        <label>Image URL</label>
                        <input type="url" name="image" class="form-control" value="<?php echo $edit_product ? ($edit_product['image']) : ''; ?>">
                    </div>
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><span class="badge <?php echo $product['status'] == 'active' ? 'badge-success' : 'badge-warning'; ?>"><?php echo $product['status']; ?></span></td>
                                <td>
                                    <a href="?edit=<?php echo $product['id']; ?>" class="btn-icon"><i class="fas fa-edit"></i></a>
                                    <a href="?delete=<?php echo $product['id']; ?>" class="btn-icon btn-danger" onclick="return confirm('Are you sure?')"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
